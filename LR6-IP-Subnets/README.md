...
- `E = 1` - показать, что MAC-адрес назначения в ARP-запросе - это MAC шлюза, а не удалённого хоста.
- `E = 2` - показать, что IP-адрес назначения в ICMP-пакете - это адрес удалённого хоста, а не шлюза.
- `E = 3` - сравнить ARP-запрос из этой лабораторной с ARP-запросом из первой лабораторной и объяснить различие.
...
- `F = 1` - почему `PC1` не определяет MAC-адрес `PC2` напрямую.
- `F = 2` - почему в ARP-запросе целевым IP является адрес шлюза, а не удалённого хоста.
- `F = 3` - почему ICMP-пакет содержит адрес удалённого хоста, а не адрес шлюза.
- `F = 4` - чем отличается логика ARP и логика IP-маршрутизации на примере данного стенда.
