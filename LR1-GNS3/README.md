...
![Пример ARP Request в Wireshark](./assets/arp_request_cropped.png)

![Пример ARP Reply в Wireshark](./assets/arp_reply_cropped.png)
...
![Пример ICMP Echo Request в Wireshark](./assets/icmp_request_cropped.png)

![Пример ICMP Echo Reply в Wireshark](./assets/icmp_reply_cropped.png)
...
![Рабочая область GNS3 с первым стендом](./assets/topology_and_ip_setup.png)
...
Для простых учебных стендов, содержащих только VPCS и встроенный `Ethernet Switch`, обычно достаточно локальной установки GNS3. Для более сложных стендов может потребоваться GNS3 VM.
...
- [Проект GNS3](./gns3/)
- [Конфигурации VPCS](./configs/)
- [Файл захвата](./pcap/)
- [Полный комплект скриншотов](./screenshots/)
- [Локальные иллюстрации из README](./assets/)

Основные иллюстрации встроены прямо в текст отчёта выше, а полный набор вспомогательных скриншотов находится в каталоге `screenshots`.
...
### Задание 1. Подготовка среды и построение стенда
...
### Задание 2. Проверка связности и получение захвата
...
### Задание 3. Анализ захваченного трафика
...
|---|---|
| 1 | Объяснить, почему `ARP Request` является широковещательным кадром |
| 2 | Сравнить MAC-адрес назначения в `ARP Request` и `ICMP Echo Request` |
...
| 5 | Вручную описать инкапсуляцию одного `ICMP Echo Request` |
